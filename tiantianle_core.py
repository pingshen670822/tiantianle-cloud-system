...
def backtest(draws, rounds=None, top_sizes=(5, 10, 15)):
    rounds = runtime_rounds("TIANTIANLE_CORE_BACKTEST_ROUNDS", 120) if rounds is None else rounds
    if len(draws) < 130:
...
def latest_settled_prediction(db_path=DB_PATH):
    try:
        with sqlite3.connect(db_path) as conn:
            row = conn.execute(
                """
                SELECT based_on_date, target_date, actual_date,
...
def backtest_two_stage_group_model(draws, windows=None):
    if windows is None:
        windows = (
            runtime_rounds("TIANTIANLE_GROUP_BACKTEST_SHORT", 60, 30, 240),
            runtime_rounds("TIANTIANLE_GROUP_BACKTEST_MID", 120, 60, 360),
            runtime_rounds("TIANTIANLE_GROUP_BACKTEST_LONG", 240, 120, 720),
        )
    results = {}
...
def build_strong_prediction_packs(candidates):
    ...
    return {
        ...
        "three_hit_two": pack("\u6700\u5f373\u4e2d2~3", 2, three),
...
def render_markdown(analysis):
    ...
    pack_labels = [
        ("strong_single", "\u6700\u5f37\u55ae\u652f"),
        ("two_hit_one", "\u6700\u5f372\u4e2d1"),
        ("three_hit_two", "\u6700\u5f373\u4e2d2~3"),
        ("five_hit_two", "\u6700\u5f375\u4e2d2"),
        ("nine_hit_three", "\u6700\u5f379\u4e2d3"),
    ]
    for key, label in pack_labels:
        pack = analysis["strong_prediction_packs"].get(key, {})
...
def main():
    parser = argparse.ArgumentParser(description="\u5929\u5929\u6a02\u958b\u734e\u5f8c\u7d71\u8a08\u5206\u6790")
...
def _tiantianle_pack_probability(pool_size, hit_goal):
    if pool_size <= 0 or hit_goal <= 0:
        return {"hit_goal": hit_goal, "pool_size": pool_size, "probability": 0.0, "odds_1_in": None}
    total = math.comb(39, 5)
    success = 0
    for hit_count in range(hit_goal, min(pool_size, 5) + 1):
        success += math.comb(pool_size, hit_count) * math.comb(39 - pool_size, 5 - hit_count)
...
def _tiantianle_normalize_pack_keys(packs):
    normalized = dict(packs or {})
    if "three_hit_one" in normalized and "three_hit_two" not in normalized:
        alias = dict(normalized["three_hit_one"])
        alias["name"] = "\u6700\u5f373\u4e2d2~3"
        alias["hit_goal"] = max(2, int(alias.get("hit_goal", 1) or 1))
        normalized["three_hit_two"] = alias
    normalized.pop("three_hit_one", None)
    for key, pack in normalized.items():
        numbers = pack.get("numbers", [])
        goal = int(pack.get("hit_goal", 1) or 1)
        probability = pack.get("theoretical_probability")
        probability_goal = probability.get("hit_goal") if isinstance(probability, dict) else None
        probability_pool = probability.get("pool_size") if isinstance(probability, dict) else None
        if probability_goal != goal or probability_pool != len(numbers):
            pack["theoretical_probability"] = _tiantianle_pack_probability(len(numbers), goal)
        pack.setdefault("pool_size", len(numbers))
    return normalized
...
def compute_tiantianle_core_analysis(draws, review=None, industrial=None):
    ...
    return {
        ...
        "component_names": list(DEFAULT_MODEL_WEIGHTS.keys()),
        "weights": weights,
        "backtest": bt,
        "relationships": relationship_analysis(draws),
        "industrial_engine": industrial,
