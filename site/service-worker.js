const CACHE_NAME = 'tiantianle-ironlaw-20260624200552';
...
self.addEventListener('install', event => {
  event.waitUntil(caches.open(CACHE_NAME).then(cache => cache.addAll(APP_SHELL.map(url => url + '?v=20260624200552')).catch(() => cache.addAll(APP_SHELL))));
  self.skipWaiting();
});
...
self.addEventListener('fetch', event => {
  ...
  if (isFreshFile) {
    url.searchParams.set('v', '20260624200552');
