const CACHE_NAME = 'tiantianle-ironlaw-20260728220453';
...
self.addEventListener('install', event => {
  event.waitUntil(caches.open(CACHE_NAME).then(cache => cache.addAll(APP_SHELL.map(url => url + '?v=20260728220453')).catch(() => cache.addAll(APP_SHELL))));
  self.skipWaiting();
});
...
self.addEventListener('fetch', event => {
  ...
  const stableReportUrl = new URL('reports/complete_report.html?v=20260728220453', self.registration.scope).toString();
  ...
  if (isFreshFile) {
    url.searchParams.set('v', '20260728220453');
